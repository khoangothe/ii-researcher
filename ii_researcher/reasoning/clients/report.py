...
class ReportBuilder:
    ...
    def generate_advance_report(self, tool_history: ToolHistory, trace: Trace) -> str:
        ...
        try:
            ...
            for i, subtopic in enumerate(subtopics, 1):
                ...
                content_from_previous_subtopics += f"\n\n{self._generate_subtopic_report(trace, subtopic, content_from_previous_subtopics, subtopics)}"
    ...
    async def generate_advance_report_stream(self, tool_history: ToolHistory, trace: Trace, callback: Optional[Callable[[str], None]] = None) -> str:
        try:
            subtopics = await self._generate_subtopcis_stream(trace)
            introduction = await self._generate_introduction_stream(trace, callback)
            content_from_previous_subtopics = introduction
            for subtopic in subtopics:
                content_from_previous_subtopics += f"\n\n{await self._generate_subtopic_report_stream(trace, subtopic, content_from_previous_subtopics, subtopics, callback)}"
    ...
    async def _generate_introduction_stream(self, trace: Trace, callback: Optional[Callable[[str], None]] = None) -> str:
        try:
            messages = [
                {"role": "system", "content": "You are a professional writer. Please write a detailed report introduction on the topic. "},
                {"role": "user", "content": self.config.generate_introduction_prompt(trace.to_string(), trace.query)},
            ]
    ...
    async def _generate_subtopcis_stream(self, trace: Trace) -> List[str]:
        try:
           messages = [
                {"role": "system", "content": "You are a professional writer. Please generate a list of subtopics that can answer the main topic and relevant to the provided research data."},
                {"role": "user", "content": self.config.generate_subtopics_prompt(trace.to_string(), trace.query)},
            ]
    ...
    async def _generate_subtopic_report_stream(self, trace: Trace, current_subtopic: str, content_from_previous_subtopics: str, subtopics: list[str], callback: Optional[Callable[[str], None]] = None) -> str:
        try:
            prompt = self.config.generate_subtopic_report_prompt(trace.to_string(), content_from_previous_subtopics, subtopics, current_subtopic, trace.query)
            logging.info(f"Generating subtopic report: {current_subtopic}")
            messages = [
                {"role": "system", "content": "You are a professional writer. Please write a detailed report on the subtopic."},
    ...
    async def _generate_references_stream(self, tool_history: ToolHistory, callback: Optional[Callable[[str], None]] = None) -> str:
    ...
    async def _generate_stream(self, messages: List[Dict[str, Any]], callback: Optional[Callable[[str], None]] = None) -> str:
    ...
    def _generate_introduction(self, trace: Trace) -> str:
        ...
        try:
            messages = [
                ...
                {
                    "role": "user", 
                    "content": self.config.generate_introduction_prompt(trace.to_string(), trace.query)
                },
            ]
            return self._generate_response(messages)
        except Exception as e:
    ...
    def _generate_subtopics(self, trace: Trace) -> List[str]:
        ...
        try:
            messages = [
                ...
                {
                    "role": "user", 
                    "content": self.config.generate_subtopics_prompt(trace.to_string(), trace.query)
                },
            ]
    ...
    def _generate_subtopic_report(self, trace: Trace, current_subtopic: str, 
                                content_from_previous_subtopics: str, subtopics: List[str]) -> str:
        ...
        """
        try:
            prompt = self.config.generate_subtopic_report_prompt(
                trace.to_string(), 
